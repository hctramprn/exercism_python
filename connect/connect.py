...
class ConnectGame:
    def __init__(self, board):
        ...
        self.x_start = set([coord for coord in self.x_positions if coord[1] == 0])
        self.x_end = set([coord for coord in self.x_positions if coord[1] == len(self.lines[0]) - 1])
        self.o_start = set([coord for coord in self.o_positions if coord[0] == 0])
        self.o_end = set([coord for coord in self.o_positions if coord[0] == len(self.lines) - 1])
        ...
        if self.x_start and self.x_end:
            ...
            while new_additions:
                ...
                for coord in new_additions:
                    upper = [(coord[0] - 1, coord[1]), (coord[0] - 1, coord[1] + 2)]
                    side = [(coord[0], coord[1] - 2), (coord[0], coord[1] + 2)]
                    lower = [(coord[0] + 1, coord[1] - 2), (coord[0] + 1, coord[1])]
                ...
                new_additions = (self.x_positions.intersection(possible_nodes)).difference(x_path)
                # updates the x_path
                x_path.update(new_additions)
                if x_path.intersection(self.x_end):
                    return 'X'
        
        elif self.o_start and self.o_end:
            o_path = self.o_start
            ...
            while new_additions:
                ...
                new_additions = (self.o_positions.intersection(possible_nodes)).difference(o_path)
                # updates the x_path
